...
export default function ArtistsAndFoundersDirectoryPage() {
  return (
    <DirectoryPageShell
      eyebrow="Artists & Founders"
      title="Discover Cross-Disciplinary Web3 Builders"
      description="Browse people who are both artists and founders, bridging creative vision with community and product building."
    >
      <div className="mb-6 flex flex-wrap items-center gap-3">
        <div className="rounded-full border border-[#FFCC00]/20 bg-[#FFCC00]/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.24em] text-[#FFCC00]">
          {artistsAndFounders.length} profiles
        </div>
      </div>

      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {artistsAndFounders.map((person) => (
          <Link
            key={person.id}
            href={`/artists-and-founders/${person.id}`}
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-xs uppercase tracking-[0.24em] text-black/60 dark:text-white/40">Artist + Founder</p>
                <h2 className="mt-2 text-base font-bold tracking-tight">{person.displayName}</h2>
              </div>
              <ArrowUpRight className="h-4 w-4 text-black/40 dark:text-white/40 transition group-hover:text-[#FFCC00]" />
            </div>

            <p className="mt-4 text-sm leading-6 text-black/70 dark:text-white/70">{person.knownFor}</p>
