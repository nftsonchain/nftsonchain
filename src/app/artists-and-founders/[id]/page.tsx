...
import { artistsAndFounders } from "@/data/artistsAndFounders";
...
export default async function ArtistsAndFoundersProfilePage({ params }: Props) {
  const { id } = await params;
  const person = artistsAndFounders.find((item) => item.id === id);
  ...
  return (
    <DirectoryPageShell
      eyebrow="Artist & Founder Profile"
      title={person.displayName}
      description={person.knownFor}
      backHref="/artists-and-founders"
      backLabel="Back to artists and founders directory"
    >
      <div className="grid gap-6 lg:grid-cols-[0.85fr_1.15fr]">
        <div className="rounded-3xl border border-white/10 bg-black/20 p-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs uppercase tracking-[0.3em] text-black/60 dark:text-white/40">Artist & Founder</p>
              <h2 className="text-xl font-bold tracking-tight">{person.displayName}</h2>
            </div>
          </div>

          <div className="mt-6 rounded-2xl border border-white/10 bg-white/[0.03] p-4">
            <p className="text-xs uppercase tracking-[0.24em] text-black/60 dark:text-white/40">Known For</p>
            <p className="mt-2 text-sm leading-7 text-black/70 dark:text-white/70">{person.knownFor}</p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-2xl border border-[#FFCC00]/20 bg-[#FFCC00]/10 p-5">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-lg font-semibold text-[#FFCC00]">Cross-Disciplinary Profile</h3>
            </div>
            <p className="mt-3 text-sm leading-7 text-black/75 dark:text-white/75">
              This profile highlights a figure known for both artistic contribution and entrepreneurial leadership within Web3.
            </p>
          </div>
        </div>
      </div>
