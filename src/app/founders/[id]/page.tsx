...
export default async function FounderProfilePage({ params }: Props) {
  ...
  const founder = founders.find((item) => item.id === id);
  ...
  return (
    <DirectoryPageShell
      ...
      title={founder.displayName}
      description={founder.knownFor}
      backHref="/founders"
      backLabel="Back to founder directory"
    >
      <div className="grid gap-6 lg:grid-cols-[0.85fr_1.15fr]">
        <div className="rounded-3xl border border-white/10 bg-black/20 p-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs uppercase tracking-[0.3em] text-black/60 dark:text-white/40">NFT Founder</p>
              <h2 className="text-xl font-bold tracking-tight">{founder.displayName}</h2>
            </div>
          </div>

          <div className="mt-6 rounded-2xl border border-white/10 bg-white/[0.03] p-4">
            <p className="text-xs uppercase tracking-[0.24em] text-black/60 dark:text-white/40">Known For</p>
            <p className="mt-2 text-sm leading-7 text-black/70 dark:text-white/70">{founder.knownFor}</p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-2xl border border-[#FFCC00]/20 bg-[#FFCC00]/10 p-5">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-lg font-semibold text-[#FFCC00]">Builder Spotlight</h3>
            </div>
            <p className="mt-3 text-sm leading-7 text-black/75 dark:text-white/75">
              This profile surfaces the founder’s public identity and the work they are best known for in the NFT ecosystem.
            </p>
          </div>
        </div>
      </div>
