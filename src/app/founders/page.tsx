...
export default function FoundersDirectoryPage() {
  return (
    <DirectoryPageShell
      eyebrow="Founder Directory"
      title="Discover NFT Founders"
      description="Explore the founders, builders, and operators shaping NFT communities, platforms, and the broader Web3 culture."
    >
        ...
        {founders.map((founder) => (
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-xs uppercase tracking-[0.24em] text-black/60 dark:text-white/40">Founder</p>
                <h2 className="mt-2 text-base font-bold tracking-tight">{founder.displayName}</h2>
              </div>
              <ArrowUpRight className="h-4 w-4 text-black/40 dark:text-white/40 transition group-hover:text-[#FFCC00]" />
            </div>

            <p className="mt-4 text-sm leading-6 text-black/70 dark:text-white/70">{founder.knownFor}</p>
